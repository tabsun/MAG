...
def update_one_model(ori_img, diff_map, temp_fname, best_temp_fname, darknet_model, frcnn_model, flag, start_yolo_num, start_frcnn_num, dest_num, rate, gt_bboxes):
...
if __name__ == '__main__':
    ...
    for fname in tqdm(fnames):
        ...
        best_img = Image.open(os.path.join('submit/images', fname)).convert('RGB')
        best_img = torch.ByteTensor(torch.ByteStorage.from_buffer(best_img.tobytes()))
        best_img = best_img.view(height, width, 3).transpose(0, 1).transpose(0, 2).contiguous()
        best_img = best_img.view(1, 3, height, width).float().cuda()
        best_img = best_img / 255.
        best_diff_map = best_img.cpu().numpy() - ori_img
        ...
        while(True):
            ...
            dest, best_diff_map = update_one_model(ori_img, best_diff_map, temp_fname, best_temp_fname, darknet_model, frcnn_model, flag, start_yolo_num, start_frcnn_num, dest_num, rate, gt_bboxes)
            if(not dest):
                destinations = [False, False]
            else:
